...
def run_simulation(n_rounds=5, n_splits=5, progress_callback=None, w=0.5, **kwargs):
    ...
    for t in range(n_rounds):
        ...
        if use_noise:
             noisy_reaction = np.random.normal(loc=original_reaction, scale=0.5, size=len(y))
             reaction = np.where(
                gap > 0, 
                np.maximum(0, noisy_reaction), 
                np.minimum(0, noisy_reaction)
            )
        else:
        ...
        if i < n_preds:
            ...
            frame_annotations = [
                dict(
                    ...
                    "Red dots scatter toward the top and bottom extremes, moving away from the central blue predictions.<br>" + 
                    "Consequently, the blue prediction markers also spread out widely over time.",
                    showarrow=False,
